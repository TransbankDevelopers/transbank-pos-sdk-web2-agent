...
export default class PosHandler {
  ...
  constructor(private socket: SocketIOServer, posIntegrado) {
  ...
  autoConnect(baudrate: number, eventName: string): void {
    this.pos
      .autoconnect(baudrate)
      .then((response) => {
        this.io.emit(eventName, { success: true, response });
      })
      .catch((e) => {
  ...
  multicodeSale(
    ...
    this.pos
      .multicodeSale(amount, ticket, commerceCode, true, (data) => {
